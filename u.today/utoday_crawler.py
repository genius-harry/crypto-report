...
class UTodayCrawler:
    ...
    def fetch_article_links(self):
        ...
        try:
            ...
            # Find header articles if needed
            if len(article_links) < self.max_articles:
                header_articles = soup.select("div.header-news__item a")
                print(f"Found {len(header_articles)} header news items")
                for link in header_articles:
