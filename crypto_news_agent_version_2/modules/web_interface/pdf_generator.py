...
def generate_pdf_report(report_markdown, articles, graph_image_path=None, sentiment_data=None, coinapi_data=None):
    """
    Generate a PDF report from the report content, graph image, articles, and additional data.
    
    Args:
        report_markdown: Markdown content of the report
        articles: List of article dictionaries
        graph_image_path: Path to the graph visualization image (optional)
        sentiment_data: Market sentiment data (optional)
        coinapi_data: CoinAPI indexes data (optional)
    ...
    if sentiment_data:
        story.append(Paragraph("Current Market Sentiment", styles["CustomSubtitle"]))
        story.append(HRFlowable(width="100%", thickness=1, color=colors.gray))
        story.append(Spacer(1, 0.1*inch))
        
        # Bullish section
        bullish_cryptos = sentiment_data.get('bullish', {}).get('cryptos', [])
        bullish_summary = sentiment_data.get('bullish', {}).get('summary', 'No bullish summary available')
        
        story.append(Paragraph("<b>🚀 Bullish Outlook</b>", styles["Heading3"]))
        story.append(Paragraph(bullish_summary, styles["Normal"]))
        story.append(Spacer(1, 0.1*inch))
        
        if bullish_cryptos:
            # Create a table for bullish cryptos
            data = [["Cryptocurrency", "Mentions", "Recent News"]]
            
            for crypto in bullish_cryptos:
                crypto_name = f"{crypto.get('name', 'Unknown')} "
                if crypto.get('symbol'):
                    crypto_name += f"({crypto.get('symbol')})"
                
                mentions = str(crypto.get('count', 0))
                
                # Format reasons
                reasons_text = ""
                for reason in crypto.get('reasons', []):
                    reasons_text += f"• {reason}<br/>"
                
                data.append([crypto_name, mentions, reasons_text])
            
            table = Table(data, colWidths=[1.5*inch, 0.75*inch, 3.75*inch])
        ...
        # Bearish section
        bearish_cryptos = sentiment_data.get('bearish', {}).get('cryptos', [])
        bearish_summary = sentiment_data.get('bearish', {}).get('summary', 'No bearish summary available')
        
        story.append(Paragraph("<b>📉 Bearish Outlook</b>", styles["Heading3"]))
        story.append(Paragraph(bearish_summary, styles["Normal"]))
        story.append(Spacer(1, 0.1*inch))
        
        if bearish_cryptos:
            # Create a table for bearish cryptos
            data = [["Cryptocurrency", "Mentions", "Recent News"]]
    ...
    # Add CoinAPI data section if available
    if coinapi_data:
        story.append(Paragraph("CoinAPI Indexes", styles["CustomSubtitle"]))
        story.append(HRFlowable(width="100%", thickness=1, color=colors.gray))
        story.append(Spacer(1, 0.1*inch))
        
        # Create a table for CoinAPI data
        data = [["Index ID", "Name", "Last Value", "Asset Pairs"]]
        
        for index in coinapi_data:
            index_id = index.get('index_id', 'Unknown')
            name = index.get('name', 'Unnamed Index')
            last_value = str(index.get('last_value', 'N/A'))
            
            # Format asset pairs
            asset_pairs_text = ""
            asset_pairs = index.get('asset_pairs', [])
            for i, pair in enumerate(asset_pairs[:3]):  # Show up to 3 pairs
                asset_pairs_text += f"• {pair}<br/>"
            
            if len(asset_pairs) > 3:
                asset_pairs_text += f"+ {len(asset_pairs) - 3} more<br/>"
            
            data.append([index_id, name, last_value, asset_pairs_text])
        
        table = Table(data, colWidths=[0.8*inch, 2.5*inch, 0.8*inch, 1.9*inch])
        table.setStyle(TableStyle([
        ...
        story.append(table)
        story.append(Paragraph("Data source: CoinAPI.io", styles["Italic"]))
        story.append(Spacer(1, 0.25*inch))
