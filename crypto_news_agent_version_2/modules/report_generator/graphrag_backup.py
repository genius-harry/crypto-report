...
def run_cypher_query(cypher_query: str) -> List[Dict[str, Any]]:
    """
    Run a Cypher query directly on the Neo4j database.
...
def ask_question(chain, question, chat_history=None):
    ...
    # Check if this is a comparison question between cryptocurrencies
    ...
    # Check if this is an investment-related question
    investment_keywords = ['buy', 'invest', 'investment', 'should i', 'recommend', 'purchase', 'trade', 'promising']
    is_investment_question = any(keyword in question.lower() for keyword in investment_keywords)
    
    # Check for specific crypto queries
    crypto_mentions = []
    all_cryptos = ["bitcoin", "ethereum", "btc", "eth", "crypto", "usdc", "usd coin", "tether", "usdt", "xrp", "cardano", "ada", "solana", "sol"]
    ...
    if len(crypto_mentions) == 1 and not is_comparison:
        try:
            ...
            print(f"Error in direct crypto info: {e}")
    
    # If this is a comparison, use special handling
    if is_comparison and len(crypto_mentions) >= 2:
        try:
            comparison_info = get_crypto_comparison(crypto_mentions, is_investment_question)
            if comparison_info:
    ...
    1. If this is a comparison question between cryptocurrencies:
       - Provide a balanced comparison of the cryptocurrencies mentioned ({', '.join(crypto_mentions)})
       - Compare based on recent news, market performance, and use cases
       - Include specific data points and numbers where available
       - Keep response concise (max 300 words)
       - Structure with clear headings and bullet points
       - Do not favor one over the other without evidence
    
    2. If asked for investment advice or recommendations:
       - DO NOT provide direct investment advice
       - Focus on specific numbers, data points, and concrete facts
       - Include exact dates and amounts where available
       - Keep responses concise and direct (max 250 words)
       - Structure information in clear categories with emojis
       - End with a brief disclaimer
    
    3. If asked for news or analysis:
       - Focus on specific facts and numbers
       - Include exact dates and amounts
       - Keep responses concise (max 250 words)
       - Use clear categories with emojis
    
    4. If asked about prices or trends:
       - Include specific price points and percentages
       - Mention exact dates of significant changes
       - Focus on concrete data points
       - Keep responses concise (max 250 words)
    
    5. If asked about a specific cryptocurrency:
       - Include specific metrics and numbers
       - Mention exact dates of significant events
       - Focus on concrete developments
       - Keep responses concise (max 250 words)
    
    6. If the knowledge graph doesn't have relevant information:
       - Clearly state what data is missing
       - Do not make up or speculate
       - Do not respond with "I don't know"
    
    User question: {question}
    """
    
    try:
        # Try to answer the direct question first
        try:
            response = chain.invoke({"query": enhanced_query})
            # Extract answer using the correct result key
            answer = response.get('result', '')
            
            # Check if the answer is proper
            if answer and len(answer.strip()) > 15 and "I don't know" not in answer.lower():
                # Verify the answer is complete (doesn't end abruptly)
                if answer.strip().endswith((".", "!", "?", ":", ")", "]", "}")):
                    return answer
        except Exception as e:
            print(f"Error with direct query: {e}")
        
        # If we get here, we need a better answer using direct database queries
        return get_specific_answer(question, is_investment_question, crypto_mentions, is_comparison)
    
    except Exception as e:
        print(f"Error querying GraphRAG: {e}")
        return get_fallback_answer(question, chat_history)

def get_crypto_comparison(crypto_mentions, is_investment_question=False):
    """Generate a comparison between multiple cryptocurrencies."""
    try:
        # Normalize crypto names
        normalized_cryptos = []
        for crypto in crypto_mentions:
            if crypto.lower() in ["btc", "bitcoin"]:
                normalized_cryptos.append("bitcoin")
            elif crypto.lower() in ["eth", "ethereum"]:
                normalized_cryptos.append("ethereum")
            elif crypto.lower() in ["usdc", "usd coin"]:
                normalized_cryptos.append("usd coin")
            elif crypto.lower() in ["usdt", "tether"]:
                normalized_cryptos.append("tether")
            else:
                normalized_cryptos.append(crypto)
        
        # Remove duplicates
        normalized_cryptos = list(set(normalized_cryptos))
        
        # Build a more comprehensive query for multiple cryptos
        crypto_query = """
        MATCH (c:Cryptocurrency)<-[:MENTIONS_CRYPTO]-(a:Article)
        WHERE a.date IS NOT NULL AND 
              (toLower(c.name) IN $crypto_names OR toLower(c.symbol) IN $crypto_symbols)
        WITH a, c ORDER BY a.date DESC
        RETURN c.name as name, a.title as title, a.date as date, a.summary as summary
        LIMIT 10
        """
        
        # Create list of names and symbols
        crypto_names = [c for c in normalized_cryptos]
        crypto_symbols = []
        for c in normalized_cryptos:
            if c.lower() == "bitcoin":
                crypto_symbols.append("btc")
            elif c.lower() == "ethereum":
                crypto_symbols.append("eth")
            elif c.lower() == "usd coin":
                crypto_symbols.append("usdc")
            elif c.lower() == "tether":
                crypto_symbols.append("usdt")
        
        results = run_direct_cypher(crypto_query, {"crypto_names": crypto_names, "crypto_symbols": crypto_symbols})
        if not results or len(results) == 0:
            return None
        
        # Group results by cryptocurrency
        crypto_news = {}
        for item in results:
            crypto_name = item.get('name', '')
            if crypto_name not in crypto_news:
                crypto_news[crypto_name] = []
            ...
            formatted_summary = summary[:100]
            if numbers:
                formatted_summary += f"... (Key numbers: {', '.join(numbers)})"
            else:
                formatted_summary += "..."
            
            crypto_news[crypto_name].append({
                'title': title,
                'date': date,
        ...
        if is_investment_question:
            answer = "📊 Investment Comparison:\n\n"
        else:
            answer = "🔄 Cryptocurrency Comparison:\n\n"
        
        # Add each cryptocurrency section
        for crypto_name, news_items in crypto_news.items():
            answer += f"### {crypto_name}:\n"
            
            # Add recent news
            if news_items:
                for i, item in enumerate(news_items[:3]):  # Limit to 3 news items per crypto
                    answer += f"• {item['date']}: {item['title']}\n  {item['summary']}\n"
            else:
                answer += "• No recent news found\n"
            
            answer += "\n"
        
        # Add stablecoin-specific information for USD Coin or Tether if they're in the comparison
        if "usd coin" in normalized_cryptos or "usdc" in normalized_cryptos:
            answer += "### USD Coin (USDC) Specifics:\n"
            answer += "• Stablecoin pegged 1:1 to US Dollar\n"
            answer += "• Backed by dollar-denominated assets\n"
            answer += "• Used primarily for trading, payments, and yield\n"
            answer += "• Lower volatility than non-stablecoin cryptocurrencies\n\n"
        
        if "tether" in normalized_cryptos or "usdt" in normalized_cryptos:
            answer += "### Tether (USDT) Specifics:\n"
            answer += "• Largest stablecoin by market capitalization\n"
            answer += "• Pegged 1:1 to US Dollar\n"
            answer += "• High trading volume on exchanges\n"
            answer += "• Used extensively for crypto trading pairs\n\n"
        
        # Add comparative analysis section
        answer += "### Key Differences:\n"
        
        # Compare stablecoins vs non-stablecoins if relevant
        stable_coins = ["usd coin", "usdc", "tether", "usdt"]
        has_stablecoin = any(c in normalized_cryptos for c in stable_coins)
        has_non_stablecoin = any(c not in stable_coins for c in normalized_cryptos)
        
        if has_stablecoin and has_non_stablecoin:
            answer += "• **Volatility**: Stablecoins (USDC/USDT) maintain stable value, while Bitcoin/other cryptocurrencies experience price volatility\n"
            answer += "• **Use Case**: Stablecoins primarily used for trading and payments, non-stablecoins often seen as investments/stores of value\n"
            answer += "• **Return Potential**: Non-stablecoins offer higher potential returns but with significantly higher risk\n"
            answer += "• **Backing**: Stablecoins backed by reserve assets, while Bitcoin is backed by network consensus and scarcity\n"
        else:
            answer += "• The relevant cryptocurrencies differ in market capitalization, usage patterns, and recent price performance\n"
            answer += "• Recent news coverage highlights different market dynamics for each\n"
        
        # Add disclaimer for investment questions
        if is_investment_question:
            answer += "\n⚠️ This comparison provides information only and is not financial advice. Research thoroughly before making investment decisions."
        
        return answer
    
    except Exception as e:
        print(f"Error generating crypto comparison: {e}")
        return None

def get_specific_answer(question, is_investment_question, crypto_mentions=None, is_comparison=False):
    """Get specific answer using direct Cypher queries."""
    try:
        # If it's a comparison question with multiple cryptos, use the comparison function
        if is_comparison and crypto_mentions and len(crypto_mentions) >= 2:
            comparison = get_crypto_comparison(crypto_mentions, is_investment_question)
            if comparison:
                return comparison
        
        # Try a direct Cypher query approach for common question types
        if is_investment_question:
            # For investment questions, get comprehensive market information
            market_query = """
            MATCH (a:Article)-[:MENTIONS_CRYPTO]->(c:Cryptocurrency)
            WHERE a.date IS NOT NULL
            WITH a, collect(c.name) as cryptos
            RETURN a.title as title, a.date as date, a.summary as summary, cryptos
            ORDER BY a.date DESC
            LIMIT 5
            """
            results = run_direct_cypher(market_query)
            if results and len(results) > 0:
                ...
                for item in results:
                    ...
                    date = item.get('date', '')
                    summary = item.get('summary', '')
                    cryptos = item.get('cryptos', [])
                    
                    # Extract numbers from summary
                    import re
                    numbers = re.findall(r'\$[\d,]+(?:\.\d+)?|[\d,]+(?:\.\d+)?%', summary)
                    
                    # Format the news item with numbers and dates
                    ...
                    # Categorize the news
                    if any(keyword in title.lower() for keyword in ['regulation', 'sec', 'law', 'compliance']):
                        regulatory_news.append(f"• {date}: {title}\n  {formatted_summary}")
                    elif any(keyword in title.lower() for keyword in ['upgrade', 'update', 'fork', 'network']):
                        technical_updates.append(f"• {date}: {title}\n  {formatted_summary}")
                    elif any(keyword in title.lower() for keyword in ['partnership', 'adoption', 'institution', 'company']):
                        institutional_activity.append(f"• {date}: {title}\n  {formatted_summary}")
                    else:
                        market_developments.append(f"• {date}: {title}\n  {formatted_summary}")
                
                # Format the response with sections
                if market_developments:
                    answer += "\n".join(market_developments) + "\n\n"
                if regulatory_news:
                    answer += "⚖️ Regulatory:\n" + "\n".join(regulatory_news) + "\n\n"
                if technical_updates:
                    answer += "🔧 Technical:\n" + "\n".join(technical_updates) + "\n\n"
                if institutional_activity:
                    answer += "🏢 Institutional:\n" + "\n".join(institutional_activity) + "\n\n"
                
                answer += "⚠️ Not financial advice. Do your own research."
                return answer
        
        elif crypto_mentions and len(crypto_mentions) == 1:
            # Try to get info about a specific cryptocurrency again
            crypto_info = get_direct_crypto_info(crypto_mentions[0])
            if crypto_info:
                return crypto_info
        
        elif any(keyword in question.lower() for keyword in ['news', 'latest', 'recent', 'happening']):
            # Get recent news with numbers
            news_query = """
            MATCH (a:Article)
            WHERE a.date IS NOT NULL
            ...
            """
            results = run_direct_cypher(news_query)
            if results and len(results) > 0:
                answer = "📰 Latest News:\n\n"
                for item in results:
                    title = item.get('title', '')
        ...
        # If we still don't have a good answer, use a generic response
        if is_investment_question:
            return "Based on current market data, I can't provide specific investment recommendations. Consider researching recent developments in blockchain technology, regulatory changes, and institutional adoption before making any investment decisions. Remember that cryptocurrency markets are highly volatile."
        else:
            return "I couldn't find specific data about that in my knowledge graph. Try asking about recent crypto news, specific cryptocurrencies like Bitcoin or Ethereum, or market trends."
    
    except Exception as e:
        print(f"Error getting specific answer: {e}")
...
def get_fallback_answer(question, chat_history=None):
    """Provide a reliable fallback answer when database queries fail."""
    try:
        llm = ChatOpenAI(
            model="gpt-4o",
            temperature=0.7,
        ...
        # Create a robust fallback prompt
        fallback_prompt = """
        You are a helpful cryptocurrency analyst. The question system couldn't access the latest market data, but you can still provide a helpful response based on general cryptocurrency knowledge.
        
        Respond in a clear, factual way following these guidelines:
        1. Never say "I don't know" or "I couldn't access data"
        2. Focus on general cryptocurrency principles and knowledge
        3. Use specific numbers and statistics where possible
        4. Keep responses concise but complete
        5. For investment questions, provide balanced information without direct advice
        6. Be helpful without making specific predictions
        
        For example, if asked "Should I buy Bitcoin?", don't say "I don't know" or "I can't access data." 
        Instead say something like: "Bitcoin has historically shown high volatility with significant price cycles. Investors typically consider factors like market trends, regulation developments, technological upgrades, and their personal risk tolerance. Consider researching current market conditions and consulting financial professionals before making decisions."
        """
        
        if question.lower().startswith("who are you"):
            return "I'm your cryptocurrency analysis assistant. I help answer questions about crypto markets, specific cryptocurrencies, recent developments, and general investment concepts. How can I help with your crypto-related questions today?"
        
        # Add the question
        fallback_prompt += f"\n\nQuestion: {question}"
        
        # Call the model directly for an answer
        response = llm.invoke(fallback_prompt)
        if response and hasattr(response, 'content'):
            return response.content
        else:
            # Absolute last resort
            return "I couldn't retrieve specific market data at this moment. For cryptocurrency questions, I recommend checking reliable sources like CoinDesk, CoinMarketCap, or crypto-focused financial news for the latest information."
