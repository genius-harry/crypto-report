...
NEO4J_PASSWORD = "REDACTED"  # Updated password
...
def setup_graphrag():
    ...
    try:
        ...
        graph = Neo4jGraph(
            ...
            database="neo4j"  # Explicitly specify the default database
        )
        
        # Create a graph-based QA chain with additional configuration
        ...
        # Define a direct query function that doesn't rely on schema introspection
        def get_crypto_info(question):
            driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USERNAME, NEO4J_PASSWORD))
